...
export default function Taxis({navigation}) {
    ...
    const getBicikeLJPath = async () => {
      let URL = "http://130.61.179.62:8080/ljubljana-transport/bicycle";
        axios
          .get(URL)
          .then(function (response) {
            console.log(response.data);
            setStartingStation(response.data.startingStation.fullAddress);
            setFinishStation(response.data.finishStation.fullAddress);
            setStartingStationLat(response.data.startingStation.lat);
            setFinishStationLat(response.data.finishStation.lat);
            setStartingStationLng(response.data.startingStation.lng);
            setFinishStationLng(response.data.finishStation.lng);
    ...
    return(
        ...
        <Image
            source={require("../assets/bicikelj.png")}
            style={styles.bicikeLJContainer}
        />
        <ScrollView>
            <View style={styles.directionsContainerTop}>
                <View style={{justifyContent: 'center', padding: 8, paddingLeft: 15, paddingRight: 15}}>
                    <Text style={Platform.OS === 'ios' ? styles.directionHeaderIOS : styles.directionHeaderAndroid}>1. Head to BicikeLJ station and rent a bike with Urbana.</Text>
                </View>
            ...
            <View style={styles.directionsContainer}>
                <View style={{justifyContent: 'center', padding: 8, paddingLeft: 15, paddingRight: 15}}>
                    <Text style={Platform.OS === 'ios' ? styles.directionHeaderIOS : styles.directionHeaderAndroid}>2. Ride BicikeLJ to your destination station and return the bike.</Text>
                </View>
                <View style={styles.addressContainer}>
                    <TouchableOpacity onPress={() => handleGetDirections("walking", startingStationLat, startingStationLng, finishStationLat, finishStationLng)} style={styles.directionsTextContainer}>
                        <Image source={require('../assets/location.png')} style={styles.locationImg} />
                        <Text style={{marginLeft: 10, flex: 1, flexWrap: 'wrap'}}>{finishStation}</Text>
                    </TouchableOpacity>
                    <View style={{flexDirection: 'row', alignItems: 'center', width: '30%', borderLeftWidth: 1, borderLeftColor: '#D3D3D3', marginLeft: 10, paddingLeft: 10}}>
                        <Image source={require('../assets/bike.png')} style={styles.bikeImg} />
